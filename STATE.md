...
- 公開履歴は cleanroom 方式で旧履歴から分離したクリーンな履歴のみ（以後の通常コミットを含む）
...
- [x] Phase 17: 公開履歴の件数非依存表現への補正 ※軽微な変更（表現1文の置換）のため CLAUDE.md §2 に基づき verifier 省略・success-log 追記なし
  - 受け入れ基準: ①現在のコミット総数と誤解される「10コミット」表現を除く ②公開履歴が旧履歴から分離されたクリーン履歴のみである事実を維持する ③将来コミットが増えても有効な表現にする ④公開禁止情報を追加しない ⑤STATE.md 以外を変更しない
...
- [x] Phase 17: 「現在の状態」の公開履歴表現を件数非依存（「以後の通常コミットを含む」）に置換。分離済みクリーン履歴という事実は維持、公開禁止情報の追加なし、変更は STATE.md のみ
